public class HW05_4111064232_1 extends WordChain
{
    ...
    {
        String[] splitStrings = target[1].split(" ");
        ResizableArray[] adjList = new ResizableArray[splitStrings.length];
        HashMap hashMap = new HashMap();
        String[] tokens = new String[splitStrings.length-1];
        int index = 0;
        ...
        int left = index++;
        int right = -1;
        for(int i = 1; i < splitStrings.length; i++, left = right)
        {
        ...
        Couplet[][] dp = new Couplet[4][index];
        for(int i = 0; i < 4; i++)
        {
        ...
        for(int depth = 0; depth < 3; depth++)
        {
            for(int col = 0; col < index; col++)
            {
                ...
                float neighbors = (float)(adjList[col].sumAllValue());
                for(int i = 0; i < adjList[col].size(); i++)
                {
                    float possiblity = dp[depth][col].possiblity * ((float)(adjList[col].get(i).count) / neighbors);
                    int neighborID = adjList[col].get(i).key;
